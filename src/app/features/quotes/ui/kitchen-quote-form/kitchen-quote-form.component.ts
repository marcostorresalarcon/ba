...
export class KitchenQuoteFormComponent implements OnInit, AfterViewInit {
   ...
   * Regla UX: mientras el usuario escribe NO aplicamos formato de moneda completo
   * (comas, redondeos, etc.), solo:
   * - Permitimos dígitos y un único punto decimal
   * - Limitamos a 2 decimales
   * - Sin reordenar caracteres ni mover bruscamente el cursor
   */
  protected onCurrencyInput(event: Event, controlName: 'roughQuote' | 'clientBudget'): void {
    const input = event.target as HTMLInputElement;
    // Entrada nativa: dejamos que el usuario escriba libremente y solo limpiamos caracteres inválidos.
